...
> ***Answer 1a***:  Because Faith's PD is based on evolutionary history shared between extant species, increasing species richness generally increases Faith's PD. 
> ***Answer 1b***:  Estimates of diversity may differ when communities differ in evolutionary breadth (ecological niche breadth?), meaning a community with species from the same genus will have a low FPD, while a community with species from distinct genuses should have higher FPD.
> ***Answer 1c***:  The scaling component of 0.75 indicates diminishing returns. Therefore each additional species adds less evolutionary history than the previous one, species are phylogenetically clustered, and communities are dominated by a few clades. 
...
> ***Answer 2a***:  H0 : the observed PD should fall within the distribution of PD values generated by random communities of the same richness. HA: communities are not random draws from the phylogeny and they show non‑random phylogenetic structure.
> ***Answer 2b***: The richness null model fixes species richness in every random community, therefore SES.PD reflects whether the observed PD is unusually high or low given the richness. Because richness is controlled, the SES.PD values are near 0, meaning the null model removed the strong PD vs. species richness correlation and isolated true phylogenetic structure. 
...
> ***Answer 3a***:  NRI is based on mean pairwise phylogenetic distances, comparing average phylogenetic distance between all pairs of species in a community. Because this metric considers distance across the entire phylogenetic tree, it captures patterns like clustering and overdispersion. 
> ***Answer 3b***:  NTI is based on mean nearest taxon distance, calculating average distance to each species closest relative. 
> ***Answer 3c***:  Based on the negative NRI and NTI values, species are less closely related than expected with the null model.There may be overdispersion of competitive exclusion occuring in this community.   
> ***Answer 3d***:  Done.
...
> ***Answer 4a***: MPD measures how phylogenetically different two communities are by taking the average of all pairwise distances between species in Comm A and species in Comm B. Unweighted UniFrac measures the fraction of the phylogenetic tree that is unique to each community. MPD calculates the average phylogenetic distance and UniFrac calculates unique branch distance. 
> ***Answer 4b***: In our plot, MPD and UniFrac distance are closely, positively related. 
> ***Answer 4c***: MPD may vary less because it averages phylogenetic distances across all species pairs, which smooths out differences between communities. UniFrac is sensitive to unique lineages, so even small changes in species composition can cause much larger shifts in its values.
...
tax.dist <- vegdist(comm.pa, method = "jaccard")

# Perform PCoA
tax.pcoa <- cmdscale(tax.dist, eig = TRUE, k = nrow(comm.pa) - 1)

# Extract eigenvalues
eigvals <- tax.pcoa$eig

# Calculate percent variation explained
explained <- eigvals / sum(eigvals) * 100

# Variation explained by first three axes
explained[1:3]
# 8.831095 5.420578 4.532293

explainvar1.t <- round(tax.pcoa$eig[1] / sum(tax.pcoa$eig), 3) * 100
explainvar2.t <- round(tax.pcoa$eig[2] / sum(tax.pcoa$eig), 3) * 100
explainvar3.t <- round(tax.pcoa$eig[3] / sum(tax.pcoa$eig), 3) * 100
...
> ***Answer 5***: There is more clustering in the phylogenetic ordination than the taxonomic ordination.The same amount of variance was explained by the different ordination methods. This shows that it is important to consider the role of phylogenetic diversity in community composition because the taxonomic ordination and NTI values suggest species that are most abundant in the community tend to be closely related to each other, which could be overlooked with the taxonomic ordination alone.
...
> ***Answer 6***: Based on these procedures, environmental differences correspond to small increases in phylogenetic dissimilarity, but the relationship is marginal and not strongly deterministic. This suggests that while environmental filtering may shape which bacterial lineages occur where, other processes like functional redundancy may play larger roles in determining community composition. 
er 6***: 
...
> ***Answer 7***: There is very limited phylogenetic information available about my two study species. My population genetics studies would be benefited by larger phylogenetic studies, but I hope that the few inferences I'm able to make become part of that missing foundational knowledge!